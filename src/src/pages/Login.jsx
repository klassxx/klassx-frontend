...
export default function Login() {
  ...
  // revient à cette page précise après connexion au lieu du tableau de
  // bord générique, pour ne pas faire perdre sa sélection à l'élève.
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(next || REDIRECT_BY_ROLE[me.role] || "/tableau-de-bord");
    } catch (err) {
    ...
    <AuthLayout title="Se connecter" subtitle="Retrouvez vos cours et vos matières.">
      <form onSubmit={handleSubmit}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-secondary)", textAlign: "center", margin: 0 }}>
          ...
          <Link to={next ? `/inscription?next=${encodeURIComponent(next)}` : "/inscription"}>Créer un compte</Link>
        </p>
      </form>
    </AuthLayout>
  );
