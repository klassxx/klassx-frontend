...
export default function Register() {
  ...
  // Si on arrive ici depuis un parcours interrompu (ex: choix d'un
  // forfait sur /catalogue sans être connecté — voir Catalog.jsx), on
  // revient à cette page précise après inscription au lieu du tableau
  // de bord générique, pour ne pas faire perdre sa sélection à l'élève.
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(
        next || (accountType === "teacher" ? "/enseignant" : accountType === "affiliate" ? "/parrainage" : "/tableau-de-bord")
      );
    } catch (err) {
    ...
    <AuthLayout title="Créer un compte" subtitle="Rejoignez KLASSX pour préparer votre bac.">
      <form onSubmit={handleSubmit}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-secondary)", textAlign: "center", margin: 0 }}>
          Déjà inscrit ?{" "}
          <Link to={next ? `/connexion?next=${encodeURIComponent(next)}` : "/connexion"}>Se connecter</Link>
        </p>
      </form>
    </AuthLayout>
  );
